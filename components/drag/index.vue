<template>
	<view class="drag" ref="dragDiv">
		<view class="drag_bg"></view>
		<view class="drag_text">{{ confirmWords }}</view>
			<view ref="moveDiv" class="move handler handler_bg" @touchstart="mousedownFn($event)" :class="{ handler_ok_bg: confirmSuccess }" style="position: absolute;top: 0px;left: 0px;"></view>
	</view>
</template>
...
export default {
	...
	onReady() {
		(async () => {
			let width1 = 0;
			let width2 = 0;
			let view1 = await uni.createSelectorQuery().select('.drag');
			...
			// 	)
			// 	.exec();
			this.maxwidth = width1 - this.$refs.moveDiv.clientWidth;
